...
from db import inicializar_db, conectar
...
# SUPERADMIN
# =========================
cursor.execute("SELECT 1 FROM usuarios WHERE usuario=%s", ("superadmin",))
...
for c in clientes:
    cursor.execute("""
        SELECT 1 FROM clientes WHERE identificacion=%s AND empresa_id=%s
    """, (c[5], c[6]))
...
for i in inventario:
    cursor.execute("""
        SELECT 1 FROM inventario WHERE producto=%s AND empresa_id=%s
    """, (i[0], i[2]))

    if not cursor.fetchone():
        cursor.execute("""
            INSERT INTO inventario (
                producto, stock_unidades, empresa_id
            ) VALUES (%s, %s, %s)
        """, i)
